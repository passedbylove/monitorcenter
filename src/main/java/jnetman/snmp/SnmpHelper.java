...
public class SnmpHelper {
	...
	public float getFloat(OID oid) throws TimeoutException, SnmpErrorException,
			SnmpSyntaxException {
		return Float.valueOf(getString(oid));
	}


	public float[] getFloat(OID[] oids) throws TimeoutException, SnmpErrorException,
			SnmpSyntaxException {
			String[] strings = getString(oids);
			float[] floats = new float[oids.length];
		for (int i = 0; i < oids.length; i++) {
			floats[i]= Float.valueOf(strings[i]).floatValue();
		}
		return floats;
	}
	...
	public Table getTable(OID tableOid,boolean suffixIsIndex)
	{
		...
		for (VariableBinding vb : vbs) {
			...
			OID fullOid = vb.getOid();

			if (fullOid.leftMostCompare(tableOid.size(), tableOid) != 0) {
				logger.warn("The following OID doesn't seems to belong to table "
						+ vb);
				continue;
			}
			int subIdxLength = fullOid.toIntArray().length-tableOid.size();
			// extract the last part of the OID, the index
			subIdxArr = Arrays.copyOfRange(fullOid.toIntArray(),
					tableOid.size() + subIdxLength - 1, fullOid.size());//bug?tableOid.size() + 2, fullOid.size());
			subIdxOid = new OID(subIdxArr);
			// extract the first part of the OID, the column
			colIdArr = Arrays.copyOfRange(fullOid.toIntArray(), 0,
					tableOid.size()+ subIdxLength -1);//bug?tableOid.size() + 2);
			colIdOid = new OID(colIdArr);
			logger.trace("Index = " + subIdxOid + "; Column = " + colIdOid);
//			table.putVariable(subIdxOid, colIdOid, vb.getVariable());
			table.putVariable(subIdxOid,suffixIsIndex?fullOid:colIdOid,vb.getVariable());
		}

		return table;
	}

//	public Table getTable(OID tableOid,boolean fullOid)
//	{
//		logger.trace("Table retrieval started");
//
//		VariableBinding[] vbs = snmpClient.walk(tableOid);
...
//				continue;
//			}
//			int subIdxLength = vb.getOid().toIntArray().length-tableOid.size();
//			// extract the last part of the OID, the index
//			subIdxArr = Arrays.copyOfRange(vb.getOid().toIntArray(),
//					tableOid.size() + subIdxLength - 1, vb.getOid().size());//bug?tableOid.size() + 2, vb.getOid().size());
//			subIdxOid = new OID(subIdxArr);
//			// extract the first part of the OID, the column
...
//			table.putVariable(subIdxOid, colIdOid, vb.getVariable());
//			OID index = vb.getOid();
////			Table.Row row = new Table().putVariable(index,vb);
////			table.putRow(index,row);
//			table.putVariable(subIdxOid,index,vb.getVariable());
//		}
//
//		return table;
//	}
	...
	public Table getTable(OID tableOid) {
		return getTable(tableOid,false);
	}
